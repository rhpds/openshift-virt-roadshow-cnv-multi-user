...
There is ample https://access.redhat.com/documentation/en-us/migration_toolkit_for_virtualization/2.6/html/installing_and_using_the_migration_toolkit_for_virtualization/installing-the-operator_mtv[documentation for how to install and configure the Operator.]
...
.. Filter the list of VMs by your user number by entering `{user}` in the *Filter by name* box.

.. Select the three VMs you would like to migrate.
