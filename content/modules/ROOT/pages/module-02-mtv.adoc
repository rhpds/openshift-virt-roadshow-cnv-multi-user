...
. You can click the drop-down arrow next to the name of each VM being migrated to get additional details about the stages of the migration process. Click on thearrow next to *database-{user}* to expand it's details. 
+
...
. After several minutes we will see that the migration has completed successfully. While waiting on these to complete, please move on to the next section in this lab.
+
image::2025_summer/module-02-mtv/18_Completed_VMWARE_Plan.png[link=self, window=blank, width=100%]
