...
. After a few seconds, the VM will change the status to *Migrating*. The migration process will be updated, and a few seconds later, it will return to the *Running* status, but on a new node. If you want to see it move nodes click on the Pod name on the right side of the window.
+
image::module-01-intro/23_PodLink.png[link=self, window=blank, width=100%]

. When you are on the Pod Details screen you can see the Node that the virtual machine is current assigned to. If you watch closely when it finishes the migration you can see it change nodes. Blink and you'll miss it though!
+
